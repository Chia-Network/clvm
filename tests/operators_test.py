...
OPERATOR_LOOKUP = ChainableMultiOpFn(chia_dialect_info().opcode_lookup, handle_unknown_op_softfork_ready)
...
class OperatorsTest(unittest.TestCase):
    ...
    def test_unknown_op(self):
        self.assertRaises(EvalError, lambda: OPERATOR_LOOKUP(b'\xff\xff1337', SExp.to(1337), None))
    ...
    def test_plus(self):
        ...
        self.assertEqual(OPERATOR_LOOKUP(KEYWORD_TO_ATOM['+'], SExp.to([3, 4, 5]), None)[1], SExp.to(12))
    ...
    def test_unknown_op_reserved(self):
        ...
        with self.assertRaises(EvalError):
            handle_unknown_op_softfork_ready(b"\xff\xff", SExp.null())

        for suffix in [b"\xff", b"0", b"\x00", b"\xcc\xcc\xfe\xed\xfa\xce"]:
            with self.assertRaises(EvalError):
                handle_unknown_op_softfork_ready(b"\xff\xff" + suffix, SExp.null())

        with self.assertRaises(EvalError):
            # an empty atom is not a valid opcode
            self.assertEqual(handle_unknown_op_softfork_ready(b"", SExp.null()), (1, SExp.null()))

        # a single ff is not sufficient to be treated as a reserved opcode
        self.assertEqual(handle_unknown_op_softfork_ready(b"\xff", SExp.null()), (CONCAT_BASE_COST, SExp.null()))

        # leading zeroes count, and this does not count as a ffff-prefix
        # the cost is 0xffff00 = 16776960
        self.assertEqual(handle_unknown_op_softfork_ready(b"\x00\xff\xff\x00\x00", SExp.null()), (16776961, SExp.null()))
    ...
    def test_unknown_ops_last_bits(self):
        ...
        for suffix in [b"\x3f", b"\x0f", b"\x00", b"\x2c"]:
            ...
            self.assertEqual(handle_unknown_op_softfork_ready(b"\x3c" + suffix, SExp.null()), (61, SExp.null()))
