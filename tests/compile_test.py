...
def test_simple_add():
    ...
    form = SExp.from_blob(script_bin)
    v = reduce(form, [])
    assert v == 30
...
def test_sha256():
    ...
    v = reduce(SExp.from_blob(compile_text(script_source)), [])
    assert v == hashed_value

    script_source = "(sha256 x0 x1)"
    v = reduce(SExp.from_blob(compile_text(script_source)), [blob[:3], blob[3:]])
    assert v == hashed_value


def test_nested():
    script_source = "(+ (+ 5 x0) x1)"
    v = reduce(SExp.from_blob(compile_text(script_source)), [9, 1000])
    assert v == 1014
...
def test_equal():
    ...
    v = reduce(SExp.from_blob(compile_text(script_source)), [7, 3, 4])
    assert v == 1

    script_source = "(equal x0 (+ x1 x2))"
    v = reduce(SExp.from_blob(compile_text(script_source)), [7, 3, 3])
    assert v == 0
